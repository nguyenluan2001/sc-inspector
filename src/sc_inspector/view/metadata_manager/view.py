...
class MetadataManagerView(VerticalScroll):
    ...
    def __init__(self, id: str, anndata: AnnData):
        ...
        self.metadata: List[str] = [
            (metadata, self.get_metadata_type(metadata, anndata.obs))
            for metadata in anndata.obs.columns
        ]
    ...
    def group_cluster_points(self):
        ...
        for cluster in self._cluster_names:
            ...
            points.append(obsm[cluster_mask].tolist())
        ...
        if 0 <= row_index < len(self._cluster_names):
            ...
            self.post_message(
                self.SelectedMetadata(
                    ...
                    points=[self._cluster_points[row_index]]
                    if row_index < len(self._cluster_points)
                    else [],
                )
            )
